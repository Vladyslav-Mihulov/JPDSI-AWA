...
public class OrdersDAO {
    ...
    public List<Orders> findByUser(Users user) {
        return em.createQuery(
                "SELECT o FROM Orders o LEFT JOIN FETCH o.productOrderCollection WHERE o.userIdUser = :user ",
                Orders.class)
                .setParameter("user", user)
                .getResultList();
    }
    
    public List<Orders> findAllWithItems() {
        return em.createQuery(
                "SELECT DISTINCT o FROM Orders o LEFT JOIN FETCH o.productOrderCollection",
                Orders.class)
                .getResultList();
    }

    public List<Orders> findActiveOrdersWithItems() {
        return em.createQuery(
                "SELECT DISTINCT o FROM Orders o LEFT JOIN FETCH o.productOrderCollection WHERE o.status IN (1, 2)",
                Orders.class)
                .getResultList();
    }
